...
bool TextureManager::LoadFromFile(SDL_Renderer* mainRenderer, std::string path,
...
bool TextureManager::SetColorModeTo(Uint8 r, Uint8 g, Uint8 b)
{
...
void TextureManager::Render(int centerX, int centerY, SDL_Rect* srcR, double scale)
{
	//Set rendering space and render to screen
	SDL_Rect dstR = { centerX - int(mWidth * scale / 2),
							centerY - int(mHeight * scale / 2),
							int(mWidth * scale), int(mHeight * scale) };

	//Render to screen
	SDL_RenderCopy(mRenderer, mTexture, srcR, &dstR);
}
